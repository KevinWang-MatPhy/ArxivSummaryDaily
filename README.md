...
默认分类已针对电子显微学、材料科学与凝聚态物理交叉研究进行分组扩充：
...
配置使用 `CATEGORY_GROUPS` 保存主题分组，并自动生成兼容现有检索代码的
`CATEGORIES` 列表。若需要进一步覆盖通用计算机视觉论文，可自行在
`microscopy_data_analysis` 中加入 `cs.CV`；默认不启用，以避免无关论文数量过多。

生成的 GitHub Pages 首页提供与上述分组一致的主题筛选，并支持标题与摘要搜索、
时间范围、日期排序、结果计数以及 arXiv 原始分类高级筛选。论文以卡片形式展示，
在手机和桌面设备上均可直接浏览。
